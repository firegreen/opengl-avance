...
int Application::run()
{
    ...
    dirLightData.push_back(DirectionnalLight(glm::vec4(1,1,0,1),glm::normalize(glm::vec4(1,-0.2,0.4,1))));
    ...
    for (auto iterationCount = 0u; !m_GLFWHandle.shouldClose(); ++iterationCount)
    {
        ...
        for (int i=0; i< dirLightData.size(); ++i)
        {
            ...
            if(shadow.isDirty)
            {
                ...
                const float sceneRadius = glm::length(loadedScene.bboxMin + loadedScene.bboxMax) * 0.5f;

                const glm::vec3 lightDir = glm::vec3(light.direction);
                const auto dirLightUpVector = glm::normalize(glm::cross(glm::cross(lightDir, glm::vec3(0,1,0)), lightDir));
                ...
                glBindSampler(shadow.data.shadowMap, shadow.sampler); // Tell to OpenGL what sampler we want to use on this texture unit
                ...
                shadow.data.lightViewProjMatrix *= camera.getRcpViewMatrix();

                glBindBuffer(GL_SHADER_STORAGE_BUFFER, shadowSSBO);
                p = (DirectionnalLightShadowData*) glMapBuffer(GL_SHADER_STORAGE_BUFFER, GL_WRITE_ONLY) + i;
                memcpy(p, (void*) &shadow.data, sizeof(DirectionnalLightShadowData));
                glUnmapBuffer(GL_SHADER_STORAGE_BUFFER);
        ...
        glUniform1i(uCastShadow, castShadow);
    ...
    sphere(glmlv::makeSphere(10))
{
    ...
    DirectionnalLightShadowData _shadowData[2];
    glGenBuffers(1, &shadowSSBO);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, shadowSSBO);
    glBufferStorage(GL_SHADER_STORAGE_BUFFER, sizeof(DirectionnalLightShadowData)*2, &_shadowData, GL_DYNAMIC_COPY);
