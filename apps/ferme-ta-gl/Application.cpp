...
int Application::run()
{
	...
	checkGlError();
	dirLightData.push_back(DirectionnalLight(glm::vec4(0.2,0.8,0.3,1),glm::normalize(glm::vec4(-0.3,-0.7,-0.2,0))));
	dirLightData.push_back(DirectionnalLight(glm::vec4(0.6,0.7,0.5,1),glm::normalize(glm::vec4(0.3,-0.5,0.1,0))));
	pointLightData.push_back(PointLight(glm::vec4(20,50,30,1),glm::vec4(1,1,-1,0)));

	dirLightShadows.resize(dirLightData.size());

	for (DirectionnalLightShadowMap& shadow : dirLightShadows)
		shadow.initialiaze(shadingProgram, uShadowLightMap);
	...
	for (auto iterationCount = 0u; !m_GLFWHandle.shouldClose(); ++iterationCount)
	{
		...
		{
			DirectionnalLightShadowMap& shadow = dirLightShadows[i];
	...
	sphere(glmlv::makeSphere(10))
{
	...
	glmlv::loadObj(m_AssetsRootPath / "glmlv/models/crytek-sponza/sponza.obj",loadedScene, false);
